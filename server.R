...
shinyServer(function(input, output) {
  ...
  Subsystem <- reactive({
    ...
    reliability$sub1$MTTF <-
      MTTF_Function(
        reliability$sub1$Reliability_Investment,input$C01,input$Cost_Increment1,input$Amode_fail_rate1,input$Bmode_fail_rate1,input$Bmode_FEF1
      )
    ...
    reliability$sub2$MTTF <-
      MTTF_Function(
        reliability$sub2$Reliability_Investment,input$C02,input$Cost_Increment2,input$Amode_fail_rate2,input$Bmode_fail_rate2,input$Bmode_FEF2
      )
  ...
  output$MTTF <- renderPlot({
    #reactive function, basically Main()
    plot_data <-
  ...
  output$fleetSize <- renderPlot({
    if (Subsystem()$sub1$Reliability_Investment[length(Subsystem()$sub1$Reliability_Investment)] > Subsystem()$sub2$Reliability_Investment[length(Subsystem()$sub2$Reliability_Investment)])
      X <-
        Subsystem()$sub1$Reliability_Investment
    else
  ...
  output$pt <- renderPlot({
    qq <- function (n) {
      ...
      df$a <-
        MTTF_Function(
          input$Reliability_Investment_input1,df$y,input$Cost_Increment1,df$x,input$Bmode_fail_rate1,input$Bmode_FEF1
        )
  ...
  output$ava <- renderPlot({
    plot_data <-
      data.frame(
        x = Reliability()$Reliability_Investment,y = (Reliability()$MTTF / 1800 +
                                                        .4)
      )
    p <-
